...
const appointmentsDropdownOpen  = ref(false)
...
const handleNewUsersClick = () => {
  ...
  appointmentsDropdownOpen.value = false
}

const handleAppointmentsClick = () => {
  appointmentsDropdownOpen.value = !appointmentsDropdownOpen.value
  newUsersDropdownOpen.value = false
}

const clearNotifications = () => {
  notifications.resetUsers()
  newUsersDropdownOpen.value = false
}

const clearAppointmentNotifications = () => {
  notifications.resetAppointments()
  appointmentsDropdownOpen.value = false
}
    ...
    <div class="max-w-screen-2xl mx-auto px-4 lg:px-8">
      <div class="flex h-14 items-center justify-between">
        ...
        <div class="flex items-center gap-3">
          <div class="h-8 w-8 rounded bg-blue-600 flex items-center justify-center text-white font-bold text-lg">B</div>
          <span class="text-xs text-gray-500 capitalize">{{ auth.userRoles[0]?.name || 'Invitado' }}</span>
        </div>
        ...
        <div class="hidden md:flex flex-1 max-w-xl mx-8">
          <div class="relative w-full">
            ...
            <div class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-lg">🔍</div>
          </div>
        </div>
        ...
        <div class="flex items-center gap-4 lg:gap-6">
          ...
          <div class="relative">
            <button @click="handleAppointmentsClick"
              ...
              <span v-if="notifications.newAppointmentsCount > 0"
                ...
                {{ notifications.newAppointmentsCount }}
              </span>
            </button>

            <div v-if="appointmentsDropdownOpen"
              ...
              <div class="px-4 py-2 border-b border-gray-800 flex items-center justify-between">
                <span class="text-sm font-medium text-white">📅 {{ t('appointments.title') }}</span>
                <button @click="clearAppointmentNotifications"
                  class="text-xs text-gray-400 hover:text-white transition">
                  {{ t('appointments.filters.clear') }}
                </button>
              </div>

              <div v-if="notifications.newAppointments.length === 0"
                class="px-4 py-4 text-sm text-gray-500 text-center">
                Sin notificaciones
              </div>

              <div v-for="appt in notifications.newAppointments" :key="appt.id"
                class="px-4 py-3 hover:bg-gray-800 transition flex items-center gap-3">
                <div class="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white text-sm shrink-0">
                  📅
                </div>
                <div class="flex-1 min-w-0">
                  <p class="text-sm font-medium text-white truncate">{{ appt.title }}</p>
                  <p class="text-xs text-gray-400 truncate">{{ appt.client }} · {{ appt.time }}</p>
                </div>
              </div>

              <div class="px-4 py-2 border-t border-gray-800 mt-1">
                <router-link to="/admin/appointments" @click="appointmentsDropdownOpen = false"
          ...
          <div class="relative">
            ...
            <div v-if="newUsersDropdownOpen"
              class="absolute right-0 mt-2 w-72 bg-gray-900 border border-gray-800 rounded-lg shadow-2xl py-2 z-50">
              <div class="px-4 py-2 border-b border-gray-800 flex items-center justify-between">
                <span class="text-sm font-medium text-white">{{ t('dashboard.new_registrations') }}</span>
                <button @click="clearNotifications" class="text-xs text-gray-400 hover:text-white transition">
                  {{ t('appointments.filters.clear') }}
                </button>
              </div>

              <div v-if="notifications.newUsers.length === 0"
                class="px-4 py-4 text-sm text-gray-500 text-center">
                Sin notificaciones
              </div>
