            ...
            <template v-if="!isEditing">
              ...
              <Transition name="fade-step">
                <div v-if="form.case_manager_id" class="step" :class="{ completed: form.client_id }">
                  ...
                  <div v-else class="clients-grid">
                    <div v-for="client in clients" :key="client.id" class="client-chip"
                      :class="{ selected: form.client_id === client.id }" @click="form.client_id = client.id">
                ...
                <div v-if="form.client_id" class="step" :class="{ completed: form.appointment_date }">
                  ...
                  <div class="mini-calendar">
                    ...
                    <div class="mini-cal-days">
                      <span v-for="day in calendarDays" :key="day.key" class="cal-day" :class="{
                        today: day.isToday,
                        selected: day.dateStr === form.appointment_date,
                        disabled: day.disabled,
                        available: day.available,
                      }" @click="day.available && selectDate(day.dateStr)">{{ day.day }}</span>
                    </div>
                  </div>
                </div>
              </Transition>
              ...
              <Transition name="fade-step">
                <div v-if="form.appointment_date" class="step" :class="{ completed: form.start_time }">
                  ...
                  <div v-else class="slots-grid">
                    <button v-for="slot in slotsData.slots" :key="slot.start" class="slot-btn"
                      :class="{ selected: form.start_time === slot.start }" @click="form.start_time = slot.start">{{
                        slot.label }}</button>
                  </div>
                </div>
              </Transition>
              ...
              <Transition name="fade-step">
                <div v-if="form.start_time" class="step">
                  ...
                  <textarea v-model="form.notes" class="step-textarea" rows="2"
                  ...
                  <div class="appointment-summary" v-if="form.title">
                    <div class="summary-row">
                      <span>📅</span>
                      <span>{{ formatDate(form.appointment_date) }} · {{ form.start_time }} – {{ endTimePreview
                      }}</span>
                    </div>
                    <div class="summary-row">
                      <span>👤</span>
            ...
            <template v-else>
              ...
              <div class="edit-summary">
                ...
                <div class="edit-row">
                  ...
                  <span>{{ selectedAppointment?.start_time?.slice(0, 5) }} – {{ selectedAppointment?.end_time?.slice(0,
                    5)
                    }}</span>
                </div>
              </div>
              ...
              <div class="form-group">
                ...
                <div class="status-pills">
                  <button v-for="opt in statusOptions" type="button" :key="opt.value" class="status-pill" :style="form.status === opt.value
                    ? { background: opt.color, borderColor: opt.color, color: '#fff' }
                    : { borderColor: opt.color, color: opt.color }" @click="form.status = opt.value">{{ opt.icon }}
                    {{ $t('appointments.status.' + opt.value) }}</button>
                </div>
              </div>
      ...
      <div v-if="showDeleteModal" class="modal-overlay" @click.self="showDeleteModal = false">
        <div class="modal-box">
          ...
          <div class="modal-footer">
            <button class="btn-secondary" @click="showDeleteModal = false">{{ $t('appointments.actions.cancel')
            }}</button>
...
export default {
  ...
  methods: {
    ...
    formatDate(date) {
      if (!date) return ''
      // Tomar solo los primeros 10 caracteres por si viene con timestamp
